...
@section('javascript')
    <script type="module">
        $(document).ready(function() {
            $.fn.dataTable.defaults.stateDuration = {!! $stateDuration !!};
            ...
            $('.dataTable').on('init.dt', function() {
                ...
                @if ($permisos['create'])
                    $('.btn-group-add').html(
                        '<a type="button" class="btn btn-default btn-light" href="/{!! Request::path() . '/create/' . $queryParameters !!}">{{ trans('csgtcrud::crud.agregar') }}</a>'
                    );
                @endif
                @foreach ($extraActions as $action)
                    $('.btn-group-add').append(
                        '<a type="button" class="btn btn-default btn-light" href="{!! $action['url'] !!}">{{ $action['title'] }}</a>'
                    );
                @endforeach
