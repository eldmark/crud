                ...
                "columnDefs": [{
                        ...
                        "render": function(data, type, full, meta) {
                            ...
                            @foreach ($extraButtons as $extraButton)
                                ...
                                @endphp
                                html +=
                                    '<div class="btn-group btn-group-sm"><a class="mr-1 btn btn-sm btn-outline-{{ $extraButton['class'] }}" title="{!! $extraButton['title'] !!}" href="{{ $parte1 }}' +
                                    id +
                                    '{{ $parte2 . $urlVars }}" {{ $target }} {!! $extraButton['confirm'] ? "onclick=\"return confirm(\'" . $extraButton['confirmmessage'] . "\');\"" : '' !!}><span class="{{ $extraButton['icon'] }}"></span></a></div>';
                            @endforeach
                    ...
                    @foreach ($columns as $column)
                        {
                            ...
                            @elseif ($column['type'] == 'bool')
                                ...
                                "render": function(data) {
                                    ...
                                    var text = (val == 0 ? '<i class="text-danger fa fa-times"></i>' :
                            ...
                            @elseif ($column['type'] == 'url')
                                ...
                                "render": function(data) {
                                    var val = data[{{ $loop->index }}];
                                    if (val == null) return null;
                                    return '<a href="' + val + '" target="{!! $column['target'] !!}">' +
                                        val + '</a>';
                                }
                            @else
            ...
            $('.dataTable').on('init.dt', function() {
                ...
                @if ($permisos['create'])
                    $('.btn-group-agregar').html(
                        '<a type="button" class="btn btn-default" href="/{!! Request::path() . '/create/' . $queryParameters !!}">{{ trans('csgtcrud::crud.agregar') }}</a>'
                        );
                @endif
                @foreach ($extraActions as $action)
                    $('.btn-group-agregar').append(
                        '<a type="button" class="btn btn-default" href="{!! $action['url'] !!}">{{ $action['title'] }}</a>'
                        );
                @endforeach
