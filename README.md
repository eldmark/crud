...
CRUD scaffolding for Laravel. Package version `5.6`, on branch `5.6`.

## Requirements

Package version `5.6` lives on branch `5.6`. The branch number is the package
...
The Laravel floor comes from `Arr::except()` and `$request->validate()`. Both are inferred from the framework APIs the code actually calls and
